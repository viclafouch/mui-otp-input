...
- **TypeScript 6 is blocked by API Extractor** (used by `vite-plugin-dts` for `rollupTypes`) — stay on TypeScript 5.x for the library itself
...
- TypeScript 5.x latest (remove `baseUrl`, explicit paths, bundler moduleResolution) — **do NOT upgrade to TS 6** (breaks `rollupTypes` in vite-plugin-dts)
...
After `npm run build`, inspect the `dist/` folder to ensure the build produced correct artifacts:

```bash
# List all files in dist/
ls -la dist/

# Verify the ES module bundle exists and is non-empty
wc -c dist/*.es.js

# Verify the declaration file exists and inspect its content
cat dist/index.d.ts
```

**Checks:**
- The `.es.js` bundle file must exist and have a reasonable size (not empty, not suspiciously small)
- The `index.d.ts` declaration file must exist and contain:
  - All public type exports (`MuiChipsInputProps`, `MuiChipsInputChip`, etc. — match against `src/index.tsx` exports)
  - Correct type definitions (not `any`, not empty interfaces)
  - No missing types or broken references
- No peer dependencies should appear bundled in the `.es.js` file — spot-check with `grep "react" dist/*.es.js` (should only find import statements, not bundled React code)

If the declaration file is missing types or has broken references, check `vite-plugin-dts` configuration and `rollupTypes` option.

**Known issue:** `vite-plugin-dts` with `rollupTypes: true` uses API Extractor internally. API Extractor bundles its own TypeScript version (5.7.x). **TypeScript 6 is NOT compatible** with API Extractor's rollup — it produces an empty `export { }` declaration file. Stay on TypeScript 5.x until API Extractor supports TS 6. If the `index.d.ts` contains only `export { }`, downgrade TypeScript to `^5.9` and rebuild.

### 2.4 Confirm with user

Tell the user: "Phase 2 complete — all dependencies upgraded, lint/build/tests pass, zero test warnings, build output verified. Ready for Phase 3 (codebase review)?"
...
- **NEVER remove alternative package manager instructions** (yarn, pnpm) from installation sections — consumers use different package managers. Only fix syntax or formatting, never delete install methods.
...
- **NEVER remove alternative package manager instructions** (yarn, pnpm) from installation docs or README — consumers use different package managers
